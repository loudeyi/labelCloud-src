...
| **标注质检** | `Ctrl+Shift+Q` | 通读全部标注文件，列出可疑的框，并能直接跳到那一帧 |
...
| **候选框不会偷偷写盘** | 确认（回车）之前 | 预标注候选和橙色虚线预测只存在内存里：同一帧里改了别的框，也不会把它们一起写进文件。写入被拒（磁盘上是另一种格式）会报**失败**，帧继续显示为未保存，而不是看起来已保存 |
...
### 交付前把整个文件夹查一遍（`Ctrl+Shift+Q`）

长时间标注里活下来的错误都是些无聊的错，而且往往等到用数据时才暴露。**`Ctrl+Shift+Q`**
（*辅助 → 标注质检 …*）会把整个文件夹的标注文件读一遍（只读标注，几百帧也就一会儿），
列出可疑的地方：

| 报出的问题 | 依据 |
| --- | --- |
| **尺寸和同类别的常见尺寸差太多** | 某个维度与该类别在**本数据集**里的中位数相差超过 `size_ratio`（1.6 倍）—— 标过 200 根杆的人就是这里"一根杆多大"的最好定义 |
| **重复或重叠的框** | 同类别两个框的中心距离小于 `duplicate_distance`（0.35 m），或底面重叠面积比超过 `duplicate_iou`（55%） |
| **类别要求竖直，但框是斜的** | 配置里要求竖直的类别（`_classes.json` 中按类别，或全局 `z_rotation_only`）出现了横滚或俯仰 |
| **长轴写进了 length 而不是 width** | 类别模板是线状的（`width > length`），但框的长轴落在了 `length` |
| **尺寸不可能是真的** | 任一维度小于 5 cm 或大于 60 m |
| **类别不在 `_classes.json` 里** | 类别名写错、被标注文件带进来的 |
| **框里（几乎）没有点** | 当前帧里框内点数少于 `min_points`（5）—— 唯一需要点云的规则，而它本来就已经加载了 |
| **标注文件读不出来** | JSON 坏了，或是本检查解析不了的格式（比如 `vertices` 文件） |

双击一行（或选中后按 *跳到该帧*）就能打开那一帧去修，修完按 *重新检查* 再扫一遍。
质检**只读**，不写任何文件。阈值都在 `[QUALITY]` 配置段里。
...
predict_min_point_ratio = 0.5  ; 或低于上一帧点数的这个比例就丢弃
```
...
[QUALITY]
; Ctrl+Shift+Q：尺寸与同类别中位数的允许倍数、同类两框何时算重复、
; 框内少于多少点算空框（只对当前帧检查）
size_ratio = 1.6
duplicate_iou = 0.55
...
* 关键帧插值是在**传感器坐标系**里做的：数据没有自车位姿，所以车开出去很远又大幅转弯的那一段，
  插值出来的框会偏；此时逐帧的点云核对会把这些帧跳过，而不是把框写进空气里。
* 相比理想工具还缺的：橡皮筋框选（分组编辑本身已经有了，用 `Shift`+左键），以及一个能跨重标
  稳定存在的 track id（现在这些功能靠位置识别目标，而不是靠身份）。
